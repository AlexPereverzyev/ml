        ...
        for term in self.config.search_terms:            
            for p in range(1, self.config.search_results.page_count + 1):
                ...
                filtered_map = {id: stat for id, stat in ids_map.items() 
                                if not stat['s']}
                if not filtered_map:
                    self.logger.info('ids already processed in: {0}'.format(ids_map_file))
                    continue
                for id, stat in filtered_map.items():                    
                    try:
                        ...
                        self.logger.exception('failed to fetch avatar for: {0}'.format(id))
                    else:                        
                        ...
                        if avatar is not None and not self.exists(avatar_file):                            
                            ...
                            self.logger.info('ignoring default avatar for {0}'.format(id))
