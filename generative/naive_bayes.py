...
class NaiveBayesClassifier(object):
    ...
    def __init__(self, classes, dimensions=1):
        ...
        self.word_probs = {label: [1. for _ in range(dimensions)]
    ...
    def __str__(self):
        ...
        for label in self.classes:
            result += '{0} ({1}): {2}\n'.format(
                ...
                self.probs[label],
                ', '.join(('{0:.2f}'.format(c)
                          for c in self.word_probs[label])))
        return result
    ...
    def train(self, data):
        for sample in data:
            ...
            for i in range(self.dimensions):
                self.word_probs[label][i] += sample[i]
        for label in self.classes:
            for i in range(self.dimensions):
                self.word_probs[label][i] /= (
                    self.counts[label] + self.dimensions)
